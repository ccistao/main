...
-- ==================== ACTION PROGRESS TRACKING (GIỮ LẠI) ====================
...
local function setupActionProgressTracking()
    if actionProgressConnection then
        pcall(function()
            actionProgressConnection:Disconnect()
        end)
    ...
    local function hookActionProgress()
        pcall(function()
            ...
            if not tps then 
                log("⚠️ TempPlayerStatsModule not found")
                return 
            end
            
            local ap = tps:FindFirstChild("ActionProgress")
            if not ap then
                log("⚠️ ActionProgress not found")
                return
            end
            ...
            if ap and ap:IsA("NumberValue") then
                actionProgressConnection = ap:GetPropertyChangedSignal("Value"):Connect(function()
                    pcall(function()
                        currentActionProgress = ap.Value or 0
                        
                        if isHacking then
                            local percent = math.floor((ap.Value or 0) * 100)
                ...
                log("✅ ActionProgress tracking enabled")
            end
        end)
    end
...
local function escapeBeast()
    ...
    pcall(function()
        ...
        root.CFrame = hidePlatform.CFrame + Vector3.new(0, 5, 0)
        task.wait(0.1)
        root.CFrame = hidePlatform.CFrame + Vector3.new(0, 5, 0)
    end)
end
                    ...
                    while totalAttempts < maxAttempts do
                        ...
                        for idx, pcData in ipairs(allPCs) do 
                            ...
                            if hackedPCs[pcId] then
                                log("⏭️ PC " .. pcId .. " đã hack")
                            elseif skippedPCs[pcId] then
                                log("⏭️ PC " .. pcId .. " đã skip")
                                hasSkippedPC = true
                            else
                                local progress = getPCProgress(pcData)
                                if progress >= 1 then
...
-- ==================== INITIALIZATION ====================
log("═══════════════════════════════════════")
log("AUTO HACK PC V14 - OLD LOGIC + ESP")
log("═══════════════════════════════════════")
updateCharacterReferences()
createHidePlatform()
createGUI()
